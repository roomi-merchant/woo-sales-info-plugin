...
function woo_sales_deactivate()
{
    ...
    $table_name = esc_sql($table_prefix . 'woo_sales_info');
...
function add_frontend_scripts()
{
    ...
    wp_enqueue_style('woo-style', plugin_dir_url(__FILE__) . 'assets/css/woo-sales-style.css', array());
}
...
function add_backend_scripts()
{
    ...
    wp_enqueue_style('select2', plugin_dir_url(__FILE__) . 'assets/css/select2.min.css', array(), 4.1);
...
add_action('woocommerce_product_options_general_product_data', __NAMESPACE__ . '\\add_product_option');

function add_product_option()
{
    include 'admin/product-admin-option.php';
}

add_action('woocommerce_process_product_meta', __NAMESPACE__ . '\\save_custom_product_option_field');

function save_custom_product_option_field($post_id)
{
    $custom_text_field = isset($_POST['_product_admin_option']) ? sanitize_text_field($_POST['_product_admin_option']) : '';
    update_post_meta($post_id, '_product_admin_option', $custom_text_field);
}
